...
Two exposures are kept apart ([ADR-0019](decisions/0019-the-operator-holds-create-only-secret-access.md)). At the API level, no kiln controller can read `audit-postgres`: the operator's Secret grant is `create` only, since it never reads, watches or updates the credentials it makes, and the delivery controller, chaos controller and scheduler hold no Secret role at all. Two upstream identities still can, the Crossplane core ServiceAccount and the ArgoCD application controller, an accepted residual recorded in that ADR. At the network level, the NetworkPolicy `audit-postgres-writer-only` admits TCP 5432 on the Postgres pod only from the audit service's pods, so a component that obtained the password some other way still could not connect; kind's default CNI does not enforce it, so on kind that boundary is inert and the residual is accepted only for a disposable development cluster ([ADR-0020](decisions/0020-the-network-boundary-is-inert-on-kind-and-the-residual-is-conditional.md)). The end-to-end test proves the RBAC facts and the pod specs, not the network.
