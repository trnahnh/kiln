...
- **Seeing the fault at all.** A `netem` delay is added after the destination sidecar has timed the request, and a partitioned pod reports nothing, so the abort logic reads Istio's source-reported metrics, what the meshed callers actually experienced; the target and its callers must be meshed. The workload those metrics are read for is the one Istio names, the targeted pods' owning Deployment, not their `app` label: Phase 7 found the label wrong for a canary-managed service, whose serving pods belong to `<name>-primary` while every pod carries `app=<name>`, and the experiment aborted `MetricsUnavailable` instead of scoring. No injection begins before a baseline snapshot is read, and an experiment that loses metrics for the timeout aborts rather than running blind, the same fail-closed stance Phase 4 took.
...
Prometheus, Grafana and Jaeger are owned centrally, every subsystem exports metrics and spans to the same instances rather than standing up its own. OpenTelemetry traces span the full request flow (Section "Request flow" above), so a single request can be followed from submission through policy check, provisioning, scheduling, and deployment in one trace.

**How one request becomes one trace** ([ADR-0021](decisions/0021-one-trace-per-request-via-cr-annotation-and-kafka-headers.md)). The request flow crosses three asynchronous boundaries, and each has one carrier. The REST path opens the root span and stamps its W3C `traceparent` on the CR it applies as `platform.internal/traceparent`, next to the actor annotation; the composition carries it to the `TenantDatabase` and the operator carries it onto its pod template, so every controller's spans, and the scheduler's placement of the database pod, parent on the request that caused them. Every audit record carries the publishing span in W3C Kafka headers, so the audit service's consume-and-store span joins the same trace instead of starting one. Controllers open one span per audited action, backdated to when the transition began, so spans and audit events line up one to one and the trace shows the latency of each step; Crossplane's compose is the one uninstrumented gap. An unset endpoint is a no-op tracer, so nothing changes where Jaeger is absent.
